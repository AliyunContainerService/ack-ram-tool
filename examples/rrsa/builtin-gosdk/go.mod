module github.com/AliyunContainerService/ack-ram-tool/examples/rrsa/builtin-gosdk

go 1.16

require (
	github.com/AliyunContainerService/ack-ram-tool v0.4.0
	github.com/AliyunContainerService/ack-ram-tool/pkg/credentials/alibabacloudgo/helper v0.0.0
	github.com/alibabacloud-go/darabonba-openapi v0.1.15
	github.com/alibabacloud-go/sts-20150401 v1.1.0
	github.com/alibabacloud-go/tea v1.1.17
	github.com/aliyun/alibaba-cloud-sdk-go v1.61.1681
	github.com/jmespath/go-jmespath v0.4.0 // indirect
)

replace github.com/AliyunContainerService/ack-ram-tool => ../../../

replace github.com/AliyunContainerService/ack-ram-tool/pkg/credentials/alibabacloudgo/helper v0.0.0 => ../../../pkg/credentials/alibabacloudgo/helper
