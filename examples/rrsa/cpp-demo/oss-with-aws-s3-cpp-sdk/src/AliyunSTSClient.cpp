    ...
    namespace Internal
    {
            ...
            : AWSHttpResourceClient(clientConfiguration, STS_RESOURCE_CLIENT_LOG_TAG)
        {
            ...
            if (m_aliyunOidcProviderArn.empty()) {
               AWS_LOGSTREAM_WARN(STS_RESOURCE_CLIENT_LOG_TAG, "oidc role arn must be specified to use STS AssumeRole web identity creds provider.");
               return;
            }
        ...
        AliyunSTSCredentialsClient::STSAssumeRoleWithWebIdentityResult AliyunSTSCredentialsClient::GetAssumeRoleWithWebIdentityCredentials(const STSAssumeRoleWithWebIdentityRequest& request)
        {
            ...
            //     -d "RoleArn=$RoleArn&OIDCProviderArn=$OIDCProviderArn&OIDCToken=$OIDCToken&RoleSessionName=$RoleSessionName&Version=$Version"
            ss << "Action=AssumeRoleWithOIDC"
                << "&Timestamp=" /*iso8601*/ << Aws::Utils::StringUtils::URLEncode(Aws::Utils::DateTime::Now().ToGmtString(Aws::Utils::DateFormat::ISO_8601).c_str())
                << "&Version=2015-04-01"
                << "&SignatureNonce=" << Aws::Utils::HashingUtils::HashString(Aws::Utils::StringUtils::to_string(rand()).c_str()) // something random
            ...
            *body << ss.str();
            ...
            //     </OIDCTokenInfo>
            //     <AssumedRoleUser>
            //         <Arn>acs:ram::xxxx:role/xxxx/default</Arn>
            ...
            if (!rootNode.IsNull() && (rootNode.GetName() != "AssumeRoleWithOIDCResponse")) // [aliyun]
            {
            ...
            if (!resultNode.IsNull())
            {
                ...
                if (!credentialsNode.IsNull())
                {
                    ...
                    XmlNode secretAccessKeyNode = credentialsNode.FirstChild("AccessKeySecret"); // [aliyun]
                    if (!secretAccessKeyNode.IsNull())
                    {
                        result.creds.SetAWSSecretKey(secretAccessKeyNode.GetText());
                    }

                    XmlNode sessionTokenNode = credentialsNode.FirstChild("SecurityToken"); // [aliyun]
